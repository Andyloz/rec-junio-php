<?php
    ...
    function escribir_parrafo($datos_comprobar,&$grupos)
    {
        $regs_prof=count($datos_comprobar["usuario"]);
        if($regs_prof==1)
            $profesores="<p class='centrar relleno'>Has seleccionado un aula que está siendo usada por el profesor ".$datos_comprobar["usuario"][0];
        else
        {
            $profesores="<p class='centrar relleno'>Has seleccionado un aula que está siendo usada por los profesores (";
            for($i=0;$i<$regs_prof-2;$i++)
            {
                $profesores.=$datos_comprobar["usuario"][$i].", ";
            }
            $profesores.=$datos_comprobar["usuario"][$i]." y ".$datos_comprobar["usuario"][$i+1].")";
        }
        $regs_grupos=count($datos_comprobar["nombre"]);
        if($regs_grupos==1)
        {
            $grupos2=" para el grupo ";
            $grupos=$datos_comprobar["nombre"][0];
            $grupos2.=$grupos."</p>";
        }
        else
        {

            $grupos2=" para los grupos ";
            $grupos="(";
            for($i=0;$i<$regs_grupos-2;$i++)
            {
                $grupos.=$datos_comprobar["nombre"][$i].", ";
            }
            $grupos.=$datos_comprobar["nombre"][$i]." y ".$datos_comprobar["nombre"][$i+1].")";
            $grupos2.=$grupos."</p>";
        }
        return $profesores.$grupos2;
        
    }

    if(isset($_POST["btnCambiarAula"]))
    {
        $id_grupos_cambio=explode(",",$_POST["grupo_cambio"]);
        for($i=0;$i<count($id_grupos_cambio);$i++)
        {
            $consulta="update horario_lectivo set aula=".$_POST["aula_cambio"]." where dia=".$_POST["dia"]." and hora=".$_POST["hora"]." and grupo=".$id_grupos_cambio[$i]." and aula=".$_POST["aula"];
    ...
    if(isset($_POST["btnComprobarAula"]))
    {
        ...
        if($resultado)
        {
           ...
           {
                $datos_comprobar["id_grupo"]=array();
                $datos_comprobar["nombre"]=array();
                $datos_comprobar["usuario"]=array();
                while($datos_fila=mysqli_fetch_assoc($resultado))
                {
                    if(!in_array($datos_fila["id_grupo"],$datos_comprobar["id_grupo"]))
                    {   
                        $datos_comprobar["id_grupo"][]=$datos_fila["id_grupo"];
                        $datos_comprobar["nombre"][]=$datos_fila["nombre"];
                    }
                    if(!in_array($datos_fila["usuario"],$datos_comprobar["usuario"]))
                    $datos_comprobar["usuario"][]=$datos_fila["usuario"];
                }
                if(!in_array($_POST["grupo"],$datos_comprobar["id_grupo"]))
                {
                    $contenido="<h2 class='centrar relleno'>Confirmación cambio de aula del ".$dias[$_POST["dia"]]." a ".$_POST["hora"]."º Hora</h2>";
                    $contenido.=escribir_parrafo($datos_comprobar,$grupos);
                    
                    $contenido.="<p class='centrar relleno'>Para añadirle este aula a ".$_POST["btnComprobarAula"].", debes cambiarle antes el aula a ".$grupos."</p>";
                    $contenido.="<p class='centrar relleno'><button onclick='cerrar_modal_general();'>Cancelar</button> <button onclick='montar_modal_confirmar_aula(".json_encode($datos_comprobar["id_grupo"]).",\"".$grupos."\",\"".$dias[$_POST["dia"]]."\",".$_POST["hora"].");'>Continuar</button></p>";
                        ...
                        if($i==4)
                            ...
                            for($j=1;$j<=5;$j++)
                                if(isset($horario[$j][$i]))
                                {
                                    ...
                                    echo "<form action='index.html' method='post'><input type='hidden' value='".$j."' name='dia'/><input type='hidden' value='".$i."' name='hora'/><input type='hidden' value='".$profesor."' name='profesor'/><input class='sin_boton' type='submit' name='btnEditar' value='Editar'/></form></td>";
                                }
                                else
                                    echo "<td><form action='index.html' method='post'><input type='hidden' value='".$j."' name='dia'/><input type='hidden' value='".$i."' name='hora'/><input type='hidden' value='".$profesor."' name='profesor'/><input class='sin_boton' type='submit' name='btnEditar' value='Editar'/></form></td>";
                    ...
                    if(isset($_POST["btnEditar"])|| isset($_SESSION["accion"]))
                    {
                        ...
                        if($resultado)
                        {
                            ...
                            while($datos=mysqli_fetch_assoc($resultado))
                            {
                                ...
                                echo "<tr><td>".$datos["nombre_grupo"]." (".$datos["nombre_aula"].")</td><td><form action='index.html' method='post'><button class='sin_boton' type='submit' name='btnBorrarGrupo' value='".$datos["id_horario"]."'>Quitar</button>";
